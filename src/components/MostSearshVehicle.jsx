...
function MostSearshVehicle({ loading, vehicles }) {
  ...
  const { list, status, error } = useSelector((state) => state.vehicles);
  useEffect(() => {
  ...
  if (loading)
    return (
      <div
        role="status"
        className="flex justify-center items-center bg-gray-100 bg-opacity-70 h-screen absolute inset-0"
      >
        <svg
          aria-hidden="true"
...
export default MostSearshVehicle;
